...
const SPACE_TYPES = ["podcast", "photography", "gaming"] as const;
...
export const createSpaceSchema = z.object({
  ...
  type: z.enum(SPACE_TYPES, { errorMap: () => ({ message: `Space type must be one of: ${SPACE_TYPES.join(", ")}` }) }),
...
export const updateSpaceSchema = z.object({
  ...
  type: z.enum(SPACE_TYPES, { errorMap: () => ({ message: `Space type must be one of: ${SPACE_TYPES.join(", ")}` }) }).optional(),
